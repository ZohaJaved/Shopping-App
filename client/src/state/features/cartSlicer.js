  ...
  async (arg, { rejectWithValue }) => {
    try {
      const response = await axios.get(
        "http://localhost:3001/cart/getProductsInCart"
      );
    ...
    try {
      if (!arg || !arg.productDetails) {
        ...
        return rejectWithValue(
          "Invalid argument: Product details are required"
        );
      }
      // Make the API call
      const response = await axios.post(
        "http://localhost:3001/cart/ModifyCart",
        {
...
function calculateBill(cartItems) {
  ...
  if (cartItems) {
    ...
    cartItems.forEach((item) => {
      ...
      let itemTotal = item.discountedPrice * item.quantity; // Calculate total for the item
      // Apply discount if quantity is greater than or equal to 10
      if (item.quantity >= 10) {
        let discountAmount = itemTotal * 0.2; // 20% discount, replace with individual discount of each product
...
export const cartSlice = createSlice({
  ...
  reducers: {
    ...
    add_to_cart(state, action) {
      ...
      state.items = AddToCart(
        action.payload.productDetails,
        action.payload.quantityIncrement
      );
    },
  },
