...
function OrderManage() {
  ...
  async function fetchOrderDetails() {
    try {
      const response = await axios.get(
        "http://localhost:3001/order/getOrderDetails"
      );
  ...
  async function fetchDeliveryOptions() {
    try {
      const response = await axios.get(
        "http://localhost:3001/delivery/fetchDeliveryOptions"
      );
  ...
  const handleSaveChanges = async () => {
    try {
      ...
      await axios.put(
        `http://localhost:3001/order/updateOrder/${editingOrder._id}`,
        {
  ...
  return (
    <div>
      <center>
        ...
        <h3>Manage Orders</h3>
        <div className={`page-section ${sidebar ? "blur" : ""}`}>
                  ...
                  <tr key={key}>
                    ...
                    <td>
                      {editingOrder && editingOrder._id === object._id ? (
                        ...
                        <button onClick={() => handleManageClick(object)}>
                          Manage
                        </button>
                      )}
                    </td>
